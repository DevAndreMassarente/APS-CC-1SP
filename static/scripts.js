document.addEventListener('DOMContentLoaded', function() {
    ...
    function normalizarNome(nome) {
        return nome.normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '').toLowerCase();
    }
    ...
    function atualizarLogo(time) {
        ...
        const logoPath = `/static/logoclubes/${normalizarNome(time)}.png`;
    ...
    function atualizarCoresLaterais(time) {
        fetch('/get_cores_time', {
            ...
            body: JSON.stringify({ time: normalizarNome(time) }),
        })
